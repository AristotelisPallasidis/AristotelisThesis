...
{
    /// <summary>
    /// Calls the local Python palmprint service to encode a palm into a feature vector.
    /// Registered as a singleton so the underlying <see cref="HttpClient"/> is reused.
    /// </summary>
    public class PythonPalmprintRecognitionService : IPalmprintRecognitionService
    {
        private readonly HttpClient _http;

        public PythonPalmprintRecognitionService()
        {
            _http = new HttpClient
            {
                BaseAddress = new Uri(PalmprintServiceConfig.BaseUrl),
        ...
        public async Task<float[]?> EncodeAsync(byte[] imageBytes)
        {
            try
            {
                ...
                using HttpResponseMessage response = await _http.PostAsync("/encode-palm", content);
